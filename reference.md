...
**Frequency-qualified references.** A field reference may carry an optional **frequency prefix** naming the native frequency to pull it at: `annual.income.revenue`, `quarterly.income.revenue`, `daily.price.adj_close`. A source that publishes the same field at several frequencies (e.g. annual and quarterly statements) then serves each variant distinctly, and a single block may use more than one - `quarterly.income.revenue / annual.balance.total_assets` - each aligned independently from its native frequency to the target. A **bare** field (`income.revenue`, no prefix) means the providing source's default frequency; existing models are unaffected. The prefix binds looser than nothing - it is part of the field path, so `annual.income.revenue @ fmp` pins the source too. Requesting a frequency no configured source serves is `E-FREQ-UNAVAILABLE`.
